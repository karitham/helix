...
pub fn languages_all() -> std::io::Result<()> {
    ...
    let check_binary = |cmd: Option<&str>| match cmd {
        Some(cmd) => match helix_stdx::env::which(cmd) {
            Ok(_) => color(fit(&format!("✓ {}", cmd)), ColorSpec::GREEN),
            Err(_) => color(fit(&format!("✘ {}", cmd)), ColorSpec::RED),
        },
        None => color(fit("None"), ColorSpec::YELLOW),
    };
    ...
    for lang in &syn_loader_conf.language {
        ...
        for ts_feat in TsFeature::all() {
            match load_runtime_file(&lang.language_id, ts_feat.runtime_filename()).is_ok() {
                true => write!(stdout, "{}", color(fit("✓"), ColorSpec::GREEN))?,
                false => write!(stdout, "{}", color(fit("✘"), ColorSpec::RED))?,
            }
        }
